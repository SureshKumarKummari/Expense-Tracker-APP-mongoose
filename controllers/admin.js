...
exports.login = (req, res, next) => {
  users
    ...
    .then((user) => {
      if (user) {
        bcrypt
          .checkpass(req.params.password, user.password)
          .then((passwordMatch) => {
            if (passwordMatch) {
              //console.log(user);
              ...
              res
                ...
                }); // Send user data if passwords match
            } else {
...
exports.resetPassword = async (req, res, next) => {
  try {
    ...
    if (urldata.isactive) {
      ...
      res.render(
        `<html><body><h1>The Link for Resetting password has been Expired! please generate Newone To update password!</h1></html></body>`
      );
    }
  } catch (err) {
